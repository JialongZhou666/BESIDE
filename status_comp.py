...
def feat_by_BESIDE_from_pkl(epoch_pkl_fpath, mode_choose):
    ...
    for i in range(len(epoch_emb)):
        ...
        if mode_choose == 'BESIDE_sta':
            sta_source_score_emb.append(
                np.matmul(np.matmul(tmp_emb, sta_w1_source) + sta_b1_source, sta_w_for_score) + sta_b_for_score)
            sta_target_score_emb.append(
                np.matmul(np.matmul(tmp_emb, sta_w1_target) + sta_b1_target, sta_w_for_score) + sta_b_for_score)
            ...
            sta_source_score_emb.append(
                np.matmul(np.concatenate([sss_sta, sss_bal], axis=0),
                          sta_w_for_score_combined) + sta_b_for_score_combined)

            sts_sta = np.matmul(tmp_emb, sta_w1_target) + sta_b1_target
            sts_bal = np.matmul(tmp_emb, wi) + 0.5 * bedge

            sta_target_score_emb.append(
                np.matmul(np.concatenate([sts_sta, sts_bal], axis=0),
                          sta_w_for_score_combined) + sta_b_for_score_combined)
        else:
    ...
    # sta_source_rank = np.argsort(sta_source_score_emb)
    # sta_source_rank = sta_source_rank[::-1]
    # sta_target_rank = np.argsort(sta_target_score_emb)
    # sta_target_rank = sta_target_rank[::-1]
...
def judge_by_rank_score_for_BESIDE(rk_source_score, rk_target_score, edge_test):
    ...
    for edge in edge_test:
        ...
        if (rk_source_score[u] < rk_target_score[v] and val == 1) or (
                rk_source_score[u] > rk_target_score[v] and val == -1):
            correct_num += 1
        # if (rk_source_score[u] < rk_target_score[v] and rk_target_score[u] < rk_source_score[v]) or (rk_source_score[u] > rk_target_score[v] and rk_target_score[u] > rk_source_score[v]):
        #     correct_num += 1
...
def main(dataset_choose, method_choose, dataset_train_fpath, dataset_test_fpath, dataset_nodes_fpath,
         BESIDE_para_fpath):
    sub_log_fpath = 'log/{}_{}_status_cmp_log.txt'.format(dataset_choose, method_choose)
    print('write result to log:', sub_log_fpath)

    node_train_set, G_pos_train, G_neg_train, node_num = stacmp_read_train_edge(dataset_train_fpath,
                                                                                dataset_nodes_fpath)
    ...
    if 'BESIDE' in method_choose:
        ...
        BESIDE_sta_source_score_emb, BESIDE_sta_target_score_emb = feat_by_BESIDE_from_pkl(epoch_pkl_fpath,
                                                                                           method_choose)
        ...
        with open(sub_log_fpath, 'a') as f:
            s = '{}: model: {}\n{}: acc: {:.6f}\n\n'.format(datetime.datetime.now(), BESIDE_para_fpath, method_choose,
                                                            acc)
            f.write(s)
