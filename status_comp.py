...
'''
def feat_by_BESIDE_from_pkl(epoch_pkl_fpath, mode_choose, edge_train):
    node_score = dict()
    ...
    out_pos = np.zeros(len(edge_train))
    out_neg = np.zeros(len(edge_train))
    score = np.zeros(3783+1)
    ...
    for edge in edge_train:
        u,v,val = edge[0],edge[1],edge[2]
        if val == 1:
            score[v] += sta_source_score_emb[u] / out_pos[u]
        elif val == -1:
            score[v] -= sta_target_score_emb[u] / out_neg[u]
...
def judge_by_rank_score_for_BESIDE(score,edge_test):
    ...
    for edge in edge_test:
        ...
        if (score[u] < score[v] and val == 1) or (score[u] > score[v] and val == -1): 
            correct_num += 1
        # if (rk_source_score[u] < rk_target_score[v] and rk_target_score[u] < rk_source_score[v]) or (rk_source_score[u] > rk_target_score[v] and rk_target_score[u] > rk_source_score[v]): 
        #     correct_num += 1
        if score[u] < score[v]:
...
def main(dataset_choose, method_choose, dataset_train_fpath, dataset_test_fpath, dataset_nodes_fpath, BESIDE_para_fpath):
    ...
    if 'BESIDE' in method_choose:
        ...
        # BESIDE_sta_source_score_emb, BESIDE_sta_target_score_emb = feat_by_BESIDE_from_pkl(epoch_pkl_fpath, method_choose, edge_train)
        score= feat_by_BESIDE_from_pkl(epoch_pkl_fpath, method_choose, edge_train)
        
        # acc = judge_by_rank_score_for_BESIDE(BESIDE_sta_source_score_emb, BESIDE_sta_target_score_emb, edge_test)
        acc = judge_by_rank_score_for_BESIDE(score, edge_test)
